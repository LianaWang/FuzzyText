...
class CurveResize(object):
    ...
    def _resize_masks(self, results):
        for key in results.get('mask_fields', []):
            ...
            mask = results[key]
            if self.keep_ratio:
                mask = mmcv.imrescale(mask, results['scale_factor'], interpolation='nearest')
            else:
                mask_size = (results['img_shape'][1], results['img_shape'][0])
                mask = mmcv.imresize(mask, mask_size, interpolation='nearest')
            results[key] = mask
...
class CurvePad(object):
    ...
    def _pad_masks(self, results):
        ...
        for key in results.get('mask_fields', []):
            results[key] = mmcv.impad(results[key], shape=pad_shape, pad_val=self.pad_val)
...
#     def __init__(self, final_size=640):
#         self.final_size = int(final_size)

#     def __call__(self, results):
#         img = results['img']
#         mask = results['gt_mask']
#         mask_gap = results['gt_mask_gap']
#         origin_shape = img.shape
#         x0 = origin_shape[1]/6.0
#         x1 = origin_shape[1]/6.0 * 5
#         y0 = origin_shape[0]/6.0
#         y1 = origin_shape[0]/6.0 * 5
#         x = random.randint(x0, x1)
#         y = random.randint(y0, y1)
#         crop_x1 = int(x - self.final_size/2)
#         crop_x2 = int(x + self.final_size/2 -1)
#         crop_y1 = int(y - self.final_size/2)
#         crop_y2 = int(y + self.final_size/2 -1)
        
#         # pad the image
#         pad_dims = ((max(-crop_y1, 0), max(crop_y2-img.shape[0], 0)),
#                     (max(-crop_x1, 0), max(crop_x2-img.shape[1], 0)),
#                     (0, 0))
#         pad_dims_mask = ((max(-crop_y1, 0), max(crop_y2-img.shape[0], 0)),
#                     (max(-crop_x1, 0), max(crop_x2-img.shape[1], 0)))
#         img = np.pad(img, pad_dims, 'constant', constant_values=0)
#         mask = np.pad(mask, pad_dims_mask, 'constant', constant_values=0)
#         mask_gap = np.pad(mask_gap, pad_dims_mask, 'constant', constant_values=0)
#         y1 = max(crop_y1, 0)
#         x1 = max(crop_x1, 0)
#         img = img[y1:y1+self.final_size, x1:x1+self.final_size, :]
#         mask = mask[y1:y1+self.final_size, x1:x1+self.final_size]
#         mask_gap = mask_gap[y1:y1+self.final_size, x1:x1+self.final_size]
#         img_shape = img.shape
#         results['img'] = img
#         results['gt_mask'] = mask
#         results['gt_mask_gap'] = mask_gap
...
class CurveRandomCrop(object):
    ...
    def __call__(self, results):
        ...
        mask = results['gt_mask']
        mask_gap = results['gt_mask_gap']
        mask_contour = results['gt_mask_contour']
        origin_shape = img.shape
        ignore_size = 8.0
        ...
        mask = np.pad(mask, pad_dims_mask, 'constant', constant_values=0)
        mask_gap = np.pad(mask_gap, pad_dims_mask, 'constant', constant_values=0)
        mask_contour = np.pad(mask_contour, pad_dims_mask, 'constant', constant_values=0)
        y1 = max(crop_y1, 0)
        x1 = max(crop_x1, 0)
        img = img[y1:y1+target_size, x1:x1+target_size, :]
        mask = mask[y1:y1+target_size, x1:x1+target_size]
        mask_gap = mask_gap[y1:y1+target_size, x1:x1+target_size]
        mask_contour = mask_contour[y1:y1+target_size, x1:x1+target_size]
        img_shape = img.shape
        results['img'] = img
        results['gt_mask'] = mask
        results['gt_mask_gap'] = mask_gap
        results['gt_mask_contour'] = mask_contour
        results['img_shape'] = img_shape
...
class CurveRandomRotate(object):
    ...
    def __call__(self, results):
        ...
        mask = cv2.warpAffine(mask, matrix, dsize=(widthNew, heightNew))
        mask_gap = cv2.warpAffine(mask_gap, matrix, dsize=(widthNew, heightNew))
        # mask_contour = cv2.warpAffine(mask_contour, matrix, dsize=(widthNew, heightNew))
        img_shape = img.shape
        results['img'] = img
        results['gt_mask'] = mask
        results['gt_mask_gap'] = mask_gap
