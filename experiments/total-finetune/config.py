model = dict(
    ...
    mask_head=dict(
        ...
        loss_gap=dict(
            ...
            loss_weight=1),
        loss_contour=dict(
            type='SmoothL1Loss', beta=1.0/9, loss_weight=0.5)),
    stuff_head=dict(
        type='PanoFpnHead',
        num_stages=4,
        in_channels=256,
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type='TotalText',
            ann_file='../../data/TotalText/ImageSets/Main/train.txt',
            img_prefix='../../data/TotalText/',
    ...
    val=dict(
        type='TotalText',
        ann_file='../../data/TotalText/ImageSets/Main/test.txt',
        img_prefix='../../data/TotalText/',
        cache_root='Cache_contour',
        pipeline=[
            dict(type='LoadImageFromFile'),
...
optimizer = dict(type='SGD', lr=0.0005, momentum=0.99, weight_decay=0.0005)
...
evaluation = dict(
    interval=600,
    proposal_thr_list=[
        0.7, 0.75, 0.81, 0.82, 0.83, 0.84, 0.85, 0.86, 0.87, 0.88
    ])
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './work_dirs'
load_from = './work_dirs/art-pretrain-total.pth'
resume_from = None
workflow = [('train', 1)]
gpu_ids = range(0, 4)
